	...
	if [[ $CHECKSOLR -gt 0 ]]
		...
		do
    			read -p "Solr services are going to be stoped. Do you wish to continue?" yn
    				case $yn in
        			[Yy]* ) /hybris/solr/bin/solr stop; break;;
				[Nn]* ) echo "Exiting..."; exit;;
        			* ) echo "Please answer yes or no.";;
    			esac
		done
...
rsync -aP $COREPATH $TARGETPATH
		...
		if [[ $TARGETVERS -eq 6 ]]
			...
			echo "config=solrconfig.xml" >> $TARGETPATH/$y/core.properties
			echo "schema=schema.xml" >> $TARGETPATH/$y/core.properties
			echo "dataDir=data" >> $TARGETPATH/$y/core.properties
			/home/hybris/scripts/solr-tools/upgradeindex/upgradeindex.sh -s -t $TARGETVERS $TARGETPATH/$y
		fi
		done

#START SOLR
/hybris/solr/bin/solr start
